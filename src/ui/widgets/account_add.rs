...
fn route_subtitle(route: &Route) -> String {
    let path = route
        ...
        .unwrap_or_default();
    format!("{}:{}{path}/emby", route.server, route.port)
}
...
impl AccountWindow {
    ...
    pub async fn add(&self) {
        ...
        let server = match normalize_server(scheme, &server) {
            Ok(server) => server,
            Err(message) => {
                imp.stack.toast(gettext(message));
        ...
        let login_client = JellyfinClient::default();
        if let Err(err) = login_client.header_change_url(&server, &port, path.as_deref()) {
            imp.stack.toast(err.to_user_facing());
            imp.stack.set_visible_child_name("entry");
            return;
        }
        ...
        let (res, discovered_server_name) = match spawn_tokio(async move {
            let res = login_client.login(&username, &password).await?;
            let discovered_server_name = if discover_server_name {
                Some(login_client.get_server_info_public().await?.server_name)
            } else {
                None
            };
            Ok::<_, anyhow::Error>((res, discovered_server_name))
        })
        .await
        {
            Ok(result) => result,
        ...
        match action_type {
            ActionType::Edit => {
                ...
                if was_active && let Err(error) = JELLYFIN_CLIENT.init(&account).await {
                    let _ = SETTINGS.edit_account(account.clone(), old_account.clone());
                    if update_preferred {
                        let _ = SETTINGS.set_preferred_server(&preferred_server);
                    }
