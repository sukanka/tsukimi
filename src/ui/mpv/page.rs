...
impl PlaybackCacheScope {
    fn current() -> Self {
        ...
        Self {
            ...
            endpoint: session.url.as_ref().map(ToString::to_string),
        }
    }
}
...
impl MPVPage {
            ...
            spawn_g_timeout(glib::clone!(
                ...
                async move {
                    ...
                    if let Err(error) = obj
                        .apply_danmaku_episode(cached.episode_id, cached.item_name, true, "cached")
                        .await
                    {
    ...
    async fn apply_danmaku_episode(
        &self, episode_id: i64, item_name: String, manual: bool, flow: &'static str,
    ) -> anyhow::Result<bool> {
        ...
        if !transaction.preserve_renderer() {
            self.clear_danmaku_result(DanmakuPopoverStatus::Loading);
        }
    ...
    fn restore_danmaku_after_manual_failure(
        ...
        self.clear_danmaku_result(fallback_status);
    }
    ...
    pub fn play(
        ...
        if resume_cached {
            ...
            } else if !should_refresh_danmaku
                && SETTINGS.mpv_danmaku_enabled()
                && !self.has_danmaku()
            {
                imp.danmaku_popover_content.set_switch_sensitive(true);
                self.auto_search_danmaku(&item);
            }
        ...
        spawn_g_timeout(glib::clone!(
            ...
            async move {
                ...
                let video_url = media_source_stream_url(media_source).await;
                if !obj.request_is_current(generation) {
                    return;
                }
