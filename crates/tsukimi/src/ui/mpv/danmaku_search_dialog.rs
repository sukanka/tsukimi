...
impl DanmakuSearchDialog {
        ...
        for mut anime in candidates.episode_animes {
            ...
            if expected_type.is_some_and(|expected_type| {
                ...
                .is_some_and(|actual_type| {
                    !Self::candidate_type_families_compatible(actual_type, expected_type)
                })
            }) {
                continue;
            }
    ...
    fn candidate_types_compatible(
        ...
        anime_type
            ...
            .is_none_or(|(anime_type, detail_type)| {
                Self::candidate_type_families_compatible(anime_type, detail_type)
            })
    }

    fn candidate_type_families_compatible(
        left: CandidateTypeFamily, right: CandidateTypeFamily,
    ) -> bool {
        left == right
            || matches!(
                (left, right),
    ...
    pub fn apply_episode(&self, item: TuItem) {
        ...
        spawn(glib::clone!(
            ...
            async move {
                match page
                    .apply_manual_danmaku(&expected_item_id, episode_id, item_name)
                    .await
                {
                    Ok(true) => {
...
mod tests {
    ...
    #[test]
    fn tmdb_and_standard_candidate_types_are_compatible() {
        assert!(DanmakuSearchDialog::candidate_type_families_compatible(
            CandidateTypeFamily::TmdbTv,
            CandidateTypeFamily::TvSeries,
        ));
        assert!(DanmakuSearchDialog::candidate_type_families_compatible(
            CandidateTypeFamily::Movie,
            CandidateTypeFamily::TmdbMovie,
        ));
        assert!(!DanmakuSearchDialog::candidate_type_families_compatible(
            CandidateTypeFamily::TmdbTv,
            CandidateTypeFamily::Movie,
